...
```{r echo=T, results='hide', message=FALSE}
# Make a factor for the condition.
condition <- as.factor(paste(annotation$Tissue, annotation$Treatment,                                              sep = "_"))
# Make a DESeq dataset matrix.
dds <- DESeqDataSetFromMatrix(countData = rawdata,
                              colData = data.frame(condition=condition),
                              design = ~ 0+condition)
# Use DESeq to calculate the DEG's.
...
g = ggplot(data=deseq.results, 
  ...
  ggtitle("Leaf_Fe vs Leaf_Control DEGs according to \n DESeq2 with FDR <= 0.05 and absolute FC >= 2") +
...
g1 = ggplot(data=deseq.results1, 
  ...
  geom_hline(yintercept = -log10(pval_threshold)) +
  xlab("fold change") + ylab("-log10 padj") +
  ggtitle("Root_Fe vs Root_Control DEGs according to \n DESeq2 with FDR <= 0.05 and absolute FC >= 2") +
  theme(plot.title = element_text(size = 8)) +
  geom_text_repel(aes(label=ifelse(padj < 0.1e-6 & abs(log2FoldChange) 
                                   >= logfc_threshold,
...
In de volcano plots die in het vorige subhoofdstuk zijn uit gewerkt is te zien dat de plots niet hetzelfde zijn. Dit betekend ook dat er verschillen zitten in het aantal DEG's. Uit een volcano plot zijn niet de absolute getallen waarneembaar. Daarom is gekozen om Venn-diagram te maken waarin het aantal DEG's per groep staan weergegeven. Een Venn-diagram is een visualisatie waarin cirkels worden gebruikt om een relatie tussen groepen weer te geven. Cirkels die elkaar overlappen hebben een overeenkomst, cirkels die elkaar niet overlappen hebben geen overeenkomst. Het doel van een Venn-diagram dus om overeenkomsten en verschillen tussen groepen visueel weer te geven.

\newpage

```{r echo=T, results='hide', message=FALSE}
# Select on the DEG's by group and threshold. 
DESeq.degs <- row.names(deseq.results[which(deseq.threshold == TRUE),])# Leafs
DESeq.degs1 <- row.names(deseq.results1[which(deseq.threshold1 == TRUE),]) # Roots
# Load package.
library(VennDiagram)
...
# Plot the data to a Venn-diagram.
...
In de Venn-diagram zullen het aantal DEG's van de Leaf groep, de Root groep en de DEG's die overlappen worden weergegeven. Uit deze Venn-diagram is gekomen dat het aantal gevonden DEG's in de Leaf groep uitkomt op 5506 stuks. In de Root groep zijn 1986 DEG's gevonden. En de DEG's die in beide groepen zijn gevonden bedragen 684 stuks.

\newpage
#### Up and down regulation calculations

Om ook iets te kunnen zeggen of de DEG's up of down gereguleerd worden moet een complexe berekening worden uitgevoerd. In een Venn-diagram kan namelijk niet duidelijk aangegeven worden hoeveel genen meer expressie vertonen en hoeveel genen minder expressie laten zien. Het is namelijk interessant om te weten hoeveel genen dit zijn zodat de data vergeleken kan worden. In de berekening voor de up-gereguleerde genen wordt de som genomen van het aantal genen dat een log2FoldChange hoger heeft dan 1. Voor de down-gereguleerde genen wordt de som genomen van het aantal genen dat een log2FoldChange lager heeft dan -1. In de code hieronder wordt het eerste deel van de berekeningen weergegeven, deze berekeningen zijn te gebruiken voor de leaf en root groep. Hiervan is een overzichtelijk dataframe gemaakt waarin te zien is hoe de verdeling van de up en down gereguleerde genen is van de leaf groep ten opzichte van de root groep. 

```{r echo=T, message=FALSE}
#Calculate how many genes are up/down regulated in the leaves
leaf_unique_up <- sum(deseq.results[setdiff(DESeq.degs, DESeq.degs1),]$log2FoldChange > 1, 
                      na.rm = T)
leaf_unique_down  <-sum(deseq.results[setdiff(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1, 
                        na.rm = T)

#Calculate how many genes are up/down regulated in the roots
root_unique_up  <- sum(deseq.results1[setdiff(DESeq.degs1, DESeq.degs),]$log2FoldChange > 1, 
                       na.rm = T)
root_unique_down  <-sum(deseq.results1[setdiff(DESeq.degs1, DESeq.degs),]$log2FoldChange < -1, 
                        na.rm = T)

df <- data.frame(Leaves = c(leaf_unique_up, leaf_unique_down),
                 Roots = c(root_unique_up, root_unique_down),
                 row.names = c("Up regulated", "Down Regulated"))

knitr::kable(df)
```

In de Venn-diagram was te zien dat tussen de leaf groep en de root groep enige overlap te zien is. Voor deze overlap moet gekeken worden naar het verschil en overeenkomst die in deze intersect groep te vinden is. Eerst is het totaal berekend van de leaves en roots in de intersect groep, zodat later deze getallen gebruikt kunnen worden om de som op te lossen. Op basis van de rownames van de root_down en leaf_down is berekend hoeveel DEG's in de root_down en in de leaf_down zaten. Deze twee getallen zijn gebruikt om de rest van de getallen te bereken. Deze complexe puzzel is op te lossen met onderstaande code. Onder de code bevindt een dataframe waarin alle up en down gereguleerde genen van de intersect groep worden weergeven voor de leaf en root groep.

```{r}
#Genes that are found in both tissues to be a DEG
#How many are in the leaves up/down regulated?
leaf_up_total <- sum(deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange > 1, 
                     na.rm = T)
leaf_down_total <-sum(deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1, 
                      na.rm = T)

#And, how many are in the roots up/down regulated
root_up_total <- sum(deseq.results1[intersect(DESeq.degs1, DESeq.degs),]$log2FoldChange > 1, 
                     na.rm = T)
root_down_total <-sum(deseq.results1[intersect(DESeq.degs1, DESeq.degs),]$log2FoldChange < -1, 
                      na.rm = T)

rownames_root_down <- row.names(deseq.results1[which(
  deseq.results1[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1),])
rownames_leaf_down <- row.names(deseq.results[which(
  deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1),])

#Calculate the amount of genes that are up/down regulated in one tissue and 
#up/down regulated in the other tissue
root_down_leaf_down <- length(intersect(rownames_leaf_down, rownames_root_down))
root_up_leaf_down <- leaf_down_total - root_down_leaf_down
root_up_leaf_up <- root_up_total - root_up_leaf_down
root_down_leaf_up <- leaf_up_total - root_up_leaf_up

df <- data.frame(leaf_up = c(root_up_leaf_up, root_down_leaf_up, leaf_up_total), 
                 leaf_down = c(root_up_leaf_down, root_down_leaf_down, leaf_down_total),
                 total = c(root_up_total, root_down_total, leaf_up_total + leaf_down_total),
                 row.names = c("root_up", "root_down", "total"))
...
Uit de mapping stappen zijn`.BAM` files verkregen. Om nu te tellen hoe vaak een read tegen een bepaald transcript gemapt is, wordt `featureCounts` gebruikt. Dit commando heeft twee soorten bestanden nodig: een annotatie bestand en alignment bestanden in `.SAM` of `.BAM` format. Met het commando hieronder wordt een count matrix voor met 6 samples gemaakt.
...
annotation_file <- "/local-fs/bachelor-students/2020-2021/Thema12/plant_gene_expression/
                    iwgsc_refseqv2.1_annotation_200916_HC.gff3"

#Paths to the .BAM files
bam_files <- paste0("/local-fs/bachelor-students/2020-2021/Thema12/plant_gene_expression/
                    raw_data/sample_SRR131146", 
                    70:75, 
...
### Stap 6: DESeq2
De laatste stap is het identificeren van genen die differentieel tot expressie komen. Het DESeq2 package wordt hiervoor gebruikt. Als input wordt de opgebouwde count matrix afkomstig van de vorige stap weer gebruikt.  
```{r, message=FALSE, fig.cap="Volcano plot van de root treatment vs root control"}
#Load the required packages
library(DESeq2)
...
knitr::kable(df, caption = "Hoeveelheid genen die differentieel gereguleerd zijn tussen de root treatment en control")
```

In figuur 3 is een volcano plot te zien van de vergelijking tussen root treatment en control. In het rood zijn de mRNA transcripten gekleurd die differentieel tot expressie komen. In totaal zijn er ruim 130 duizend mRNA transcripten opgenomen. Van deze hoeveelheid zijn 1301 omhoog gereguleerd en 1411 omlaag gereguleerd (zie tabel 6). 