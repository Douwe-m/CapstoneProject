...
```{r echo=T, results='hide', message=FALSE}
leaf_unique_up <- sum(deseq.results[setdiff(DESeq.degs, DESeq.degs1),]$log2FoldChange > 1, na.rm = T)
leaf_unique_down  <-sum(deseq.results[setdiff(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1, na.rm = T)

root_unique_up  <- sum(deseq.results1[setdiff(DESeq.degs1, DESeq.degs),]$log2FoldChange > 1, na.rm = T)
root_unique_down  <-sum(deseq.results1[setdiff(DESeq.degs1, DESeq.degs),]$log2FoldChange < -1, na.rm = T)

leaf_up_total <- sum(deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange > 1, na.rm = T)
leaf_down_total <-sum(deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1, na.rm = T)

root_up_total <- sum(deseq.results1[intersect(DESeq.degs1, DESeq.degs),]$log2FoldChange > 1, na.rm = T)
root_down_total <-sum(deseq.results1[intersect(DESeq.degs1, DESeq.degs),]$log2FoldChange < -1, na.rm = T)

rownames_root_down <- row.names(deseq.results1[which(deseq.results1[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1),])

rownames_leaf_down <- row.names(deseq.results[which(deseq.results[intersect(DESeq.degs, DESeq.degs1),]$log2FoldChange < -1),])
...
data.frame(leaf_up = c(root_up_leaf_up, root_down_leaf_up, leaf_up_total), 
